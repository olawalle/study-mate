...
const answerBuilder = (quizId, userOptionId, correctOptionId) => 
    ({quizId, userOptionId, correctOptionId, attempts: 0, showExplanation: false, alert: false})
...
export default function useTest(previousData, quizId, includeApiFunction){
    ...
    useEffect(() => {
        ...
        const prepareUserData = previousData && previousData.reduce((aggregate, current) => {
            const state = current.userOption === current.correctOption;
            const build = answerBuilder(current.quizId, current.userOption, current.correctOption, state);
    ...
    const upsertAnswer = (currentAnswer) => {
        ...
        if(answers.some(ans => ans.quizId === currentAnswer.quizId)){
            updatedAnswers = answers.map(ans => ans.quizId === currentAnswer.quizId ? currentAnswer : ans)
    ...
    const updateAnswers = () => {
        ...
        if(includeApiFunction){
            if(previousData.some(ans => ans.quizId === currentAnswer.quizId)){
                setanswersToUpdate(answersToUpdate.map(ans => ans.quizId === currentAnswer.quizId ? currentAnswer : ans))
    ...
    const userAnswersToAdd = answersToAdd.filter(answer => answer.userOptionId !== 0);
    const userAnswersToUpdate = answersToUpdate.filter(answer => answer.userOptionId !== 0);

    const userScore = answers.reduce((aggregate, current) => {
        console.log({current, aggregate})
        if(current.userOptionId === 0 || current.userOptionId !== current.correctOptionId) return aggregate
            return aggregate + 1
        }, 0);
    ...
    const answerIsCorrect = answer && (answer.userOptionId === answer.correctOptionId && answer.userOptionId !== 0)
    ...
    const correctClassName = (optionId) => {
        return (answer.attempts === 1 && answer.userOptionId === optionId 
        && answerIsCorrect 
    ...
    const willMoveNext = answer && (answer.attempts === 2 || answer.showExplanation || answerIsCorrect)
