...
export default function Quiz(props) {
    ...
    return (
        <>
            {/* {loading && <Loader />} */}
            <div className={`quiz bordered ${props.disable ? "greyed" : ""}`}>
                ...
                <button
                    disabled={!props.quiz || props.disable}
                    title={
                        props.disable ? "You need to subscribe to access this." :
                            !props.quiz
                                ? "There are no questions in this quiz. Check back later"
                                : "Click to take test"
                    }
                    className={`${props.dummy ? "tw-btn" : "blue-btn"} sm-btn`}
                    onClick={props.disable ? onSubscribe : () => onOpenModal(1)}
                >
                ...
                <button
                    ...
                    onClick={props.disable ? onSubscribe : () => onOpenModal(2)}
                >
                    {props.userquizzes && props.userquizzes.length
                        ? "Resume Learning"
                        : "Start Learning"}
